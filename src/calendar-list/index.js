...
const CalendarList = (props, ref) => {
    ...
    const renderStaticHeader = () => {
        if (shouldUseStaticHeader) {
            const onHeaderLayoutToPass = shouldMeasureHeader.current ? onHeaderLayout : undefined;
            shouldMeasureHeader.current = false;
            return (<CalendarHeader {...headerProps} testID={`${testID}.staticHeader`} style={staticHeaderStyle} month={currentMonth} addMonth={addMonth} onHeaderLayout={onHeaderLayoutToPass} showPickerButton={false} accessibilityElementsHidden={accessibilityElementsHidden} // iOS
