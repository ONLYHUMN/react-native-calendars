...
import React, { Fragment, useCallback, useMemo, forwardRef, useImperativeHandle, useRef, useState } from 'react';
...
const CalendarHeader = forwardRef((props, ref) => {
    ...
    const handleConfirmPicker = useCallback((monthIndex, year) => {
    ...
    const _renderHeader = () => {
        ...
        const TitleText = (<Text allowFontScaling={false} style={style.current.monthText} testID={`${testID}.title`} {...webProps}>
        {formatNumbers(month?.toString(monthFormat))}
      </Text>);
        const TitleContent = allowTitlePress ? (<TouchableOpacity onPress={handleOpenPicker} activeOpacity={0.7}>
        {TitleText}
      </TouchableOpacity>) : (TitleText);
        return (<Fragment>
        {TitleContent}
        {showPickerButton && (<TouchableOpacity onPress={handleOpenPicker} accessibilityRole={'button'} accessibilityLabel={'Open month and year picker'} testID={`${testID}.openPickerButton`} style={{ alignSelf: 'center', paddingHorizontal: 6, paddingVertical: 4 }}>
    ...
    return (<View testID={testID} style={propsStyle} accessible accessibilityRole={'adjustable'} accessibilityActions={accessibilityActions} onAccessibilityAction={onAccessibilityAction} accessibilityElementsHidden={accessibilityElementsHidden} // iOS
     ...
     onLayout={onHeaderLayout}>
      ...
      {renderDayNames()}
      <MonthYearPicker visible={pickerVisible} initialDate={month || new XDate()} onClose={handleClosePicker} onConfirm={handleConfirmPicker}/>
    </View>);
});
